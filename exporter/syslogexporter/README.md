...
This exporter expects the following log record attributes to be set:

| name            | type                         | description                  | RFC3164 | RFC5424 | default | example                                                         |
|-----------------|------------------------------|------------------------------|:-------:|:-------:|---------|-----------------------------------------------------------------|
| appname         | string                       | application name             |    ✅    |    ✅    | `-`     | `myproc`                                                        |
| facility        | int                          | facility number              |    ✅    |    ✅    | `1`     | `20`                                                            |
| hostname        | string                       | hostname                     |    ✅    |    ✅    | `-`     | `192.0.2.1`                                                     |
| message         | string                       | message content              |    ✅    |    ✅    | `-`     | `It's time to make the do-nuts.`                                |
| priority        | int                          | priority                     |    ✅    |    ✅    | `165`   | `165`                                                           |
| proc_id         | string                       | process_id                   |         |    ✅    | `-`     | `8710`                                                          |
| version         | int                          | syslog version               |         |    ✅    | `1`     | `1`                                                             |
| structured_data | map[string]map[string]string | list of attributes to attach |         |    ✅    | `{}`    | `{"SecureAuth@27389":{"PEN": "27389", "Realm": "SecureAuth0"}}` |

In order to prepare data structure, you can use [Transform Processor][transform_processor]. The following snippet shows minimal recommended configuration:
...
[transform_processor]: https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/transformprocessor
