...
func TestSendMultitype(t *testing.T) {
	test := prepareSenderTest(t, []func(w http.ResponseWriter, req *http.Request){
		func(w http.ResponseWriter, req *http.Request) {
			body := extractBody(t, req)
...
["lv2",13]`
	...
	test.s.buffer = exampleMultitypeLogs()
...
func TestSendJsonMultitype(t *testing.T) {
	test := prepareSenderTest(t, []func(w http.ResponseWriter, req *http.Request){
		func(w http.ResponseWriter, req *http.Request) {
			body := extractBody(t, req)
...
func TestBuffer(t *testing.T) {
	...
	droppedLogs, err := test.s.batch(context.Background(), logs[0], newFields())
	require.NoError(t, err)
	assert.Nil(t, droppedLogs)
	assert.Equal(t, 1, test.s.count())
	assert.Equal(t, []pdata.LogRecord{logs[0]}, test.s.buffer)

	droppedLogs, err = test.s.batch(context.Background(), logs[1], newFields())
	require.NoError(t, err)
	assert.Nil(t, droppedLogs)
...
func TestBufferOverflow(t *testing.T) {
	...
	for test.s.count() < maxBufferSize-1 {
		_, err := test.s.batch(context.Background(), log[0], newFields())
		require.NoError(t, err)
	}
