...
func (f *filter) prometheusTagString(record metricPair) string {
	length := record.attributes.Len()
	...
	record.attributes.ForEach(func(k string, v pdata.AttributeValue) {
		returnValue = append(
			...
			fmt.Sprintf(
				...
				f.sanitizePrometheusValue(f.convertAttributeToString(v)),
			),
		)
	})
...
func (f *filter) sanitizePrometheusKey(s string) string {
...
// `"` -> `\"`
// `\n` -> `\n`
func (f *filter) sanitizePrometheusValue(s string) string {
